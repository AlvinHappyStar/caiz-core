...
The caiz "protocol version" (a.k.a. "ledger version number") is extended to
...
soroban compiled-in to caiz-core _during_ an upgrade, we can expire one of
...
We follow this process for protocol upgrades:
  ...
  1. Before the upgrade, the network is on protocol `N`. To keep things simple,
     we require that both caiz-core and soroban give their releases version
  ...
  2. To upgrade to `N+1` we'll be producing a new soroban version `(N+1).0.0`
     and a new caiz-core `(N+1).0.0`, and the caiz-core will set its
     `Config::CURRENT_LEDGER_PROTOCOL_VERSION` to `N+1`.

  3. We configure the build of caiz-core v(N+1).0.0 as a "transitionary
     build" that has _both_ versions of soroban in it. In the `Cargo.toml` of
     caiz-core, we copy the `N.x.y` version number and git hash from the
     ...
     to `(N+1).0.0` and the appropriate git hash, and configure caiz-core
  ...
  5. We do the consensus protocol upgrade on this "transitionary build"
     ...
     caiz-core is only _willing_ to vote for a soroban-era protocol upgrade
  ...
  6. Once the upgrade is complete, we prepare the _next_ version of caiz-core
     ...
     in caiz-core's `Cargo.toml` to `(N+1).0.1`.

## Pinned soroban dependency trees

At any point we may have one _or two_ versions of soroban built into
caiz-core (see below). This is confusing and error-prone. It is easy to
...
The easiest way to update the files is just to run `caiz-core version`
...
`cargo-lock` by hand (this is the crate caiz-core uses), for example:
...
We configure soroban, caiz-core and contracts built for soroban to check
certain consistency requirements around protocol and release versions:

  1. Caiz-core's max protocol (`Config:::CURRENT_LEDGER_PROTOCOL_VERSION`)
     and its major version number must be the same.

  2. Caiz-core's and soroban's major version numbers are the same.
...
There are three caveats to the above:
  ...
  1. When caiz-core is not on a _release_ version (that is when its version
