...
### Debian packages
On Linux, the easiest is to install core using a [pre-packaged debian release](https://github.com/caiz/packages).

### Quick Start Docker image
You can use the [quickstart](https://github.com/caiz/docker-caiz-core-horizon)
image to run the latest stable and release candidate caiz-core with Docker.

### Dev container
Using vscode and the [container support in Visual Studio Code](../.devcontainer/README.md)
you can quickly build and run a caiz-core instance from your machine.
...
You can check if the build that you're using has them if `caiz-core help` returns help for the `test` subcommand:
```
...
or using the `http-command` sub-command `caiz-core http-command peers`
...
You can interact with history archives with [caiz-archivist](https://github.com/caiz/go/tree/master/tools/caiz-archivist).
...
In order to mitigate this risk, caiz-core can emit a "reference file" full of trusted hashes for
...
`METADATA_OUTPUT_STREAM` allows a caiz-core instance to stream meta data instead
...
Last history checkpoint {
    "version": 1,
    "server": "caiz-core 12.3.0 (5a8f8b13af5f78704bf0e7b5de4314c16a3639a5)",
    "currentLedger": 89663,
    "currentBuckets": [
        {
...
```
caiz-core print-xdr --base64 tx-result.txt
TransactionResult = {
  feeCharged = 100,
  result = {
    code = txSUCCESS,
...
You can generate a new Caiz public/private key with the `gen-seed` command.

```
caiz-core.exe gen-seed
...
```
caiz-core convert-id GDJLE5FAS4RVCWXAXUU226TGKXCAX7WFRNVCP75BNPWOUW7QSLKZZTUH
PublicKey:
