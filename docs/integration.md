...
caiz-core is at the bottom of the Caiz stack, many other services can be
 built on top of it.
...
# Integration points: using data generated by caiz-core

caiz-core generates several types of data that can be used by applications, depending on use cases.
...
* core's SQL database (latest only, updated every ledger)
  * in the case of captive-core (enabled via the `--in-memory` command line option) the ledger is maintained within the caiz-core process and ledger-state need to be tracked as it changes via "meta" updates.
...
As part of this transition, caiz-core records:
...
In captive-core mode, this entire per-ledger transition dataset is emitted by caiz-core over a (optionally named) pipe, as an xdr encoded `LedgerCloseMeta` object.
...
* it's easy to mix "native Caiz" data with non native data (such as internal customer information) and keep those consistent
* for more demanding applications, custom views can be built using a pub/sub model where data from caiz-core is published and consumed by many different services
...
A high resolution custom view allows to build the most sophisticated data model and derive complex data from the Caiz network.
...
Right now, caiz-core uses SQL to store the current state of the ledger.
...
* developers of Horizon have to
  *  keep track of changes in how caiz-core is storing and updating data in its database
  *  be extremely careful when performing requests as to avoid
     *  inconsistent queries
        *  caiz-core may change ledger between or while Horizon is making queries
        *  multiple caiz-core instances may not be in perfect sync with each other
     *  performance problems with poorly designed queries (non-indexed, etc).
* operationally, this creates coupling between caiz-core and Horizon:
  * a traffic spike in Horizon may cause enough stress on caiz-core that it loses sync
  * it's hard to "scale out" Horizon

### Additional reading

An overview of how the data flows within caiz-core is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that caiz-core can safely
garbage collect data no longer needed.
By default caiz-core only keeps track of its internal state and will
...
caiz-core exposes a simple HTTP based API in particular the `tx` endpoint is used to submit transactions.
