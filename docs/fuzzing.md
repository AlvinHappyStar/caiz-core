...
fuzzer with caiz-core. Support for this is still preliminary but it has
...
perform well in area (a) and with simple modifications we made to caiz-core, (b)
too. However, we've spent more energy with (c) for the `tx` fuzz mode, modifying
caiz-core such that it is more deterministic -- caches are cleared and randomness
...
## Building an instrumented caiz-core
...
to `overlay` and then `make fuzz`; this will do the following:
  ...
  - Run `caiz-core gen-fuzz fuzz-testcases/fuzz$i.xdr` ten thousand times
  ...
  - Run `afl-fuzz` on `caiz-core fuzz`, using those corpus directories

If `caiz-core fuzz` (or `afl-fuzz`) produces output such as 'Warning: AFL++
...
by feeding it to an instance of `caiz-core fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`caiz-core` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`caiz-core` in a different directory with `--enable-asan`, or valgrind, in
...
the tests). Then you can run `afl-fuzz -m 500 -M main -t 250 -i min-testcases -o fuzz-findings ./caiz-core fuzz --ll ERROR --process-id 0 --mode=${FUZZER_MODE} @@` 
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `caiz-core` itself, not libsodium,
  ...
  - Make startup-modes at different points in the process: instantiate an
    ...
    `CaizMessage`s. Let the fuzzer generte bucket ledger entries, and try to
