---
title: Commands
replacement: https://developers.caiz.org/docs/run-core-node/commands/
---

caiz-core can be controlled via the following commands.
...
* **--conf <FILE-NAME>**: Specify a config file to use. You can use 'stdin' and
  provide the config file via STDIN. *default 'caiz-core.cfg'*
...
`$ caiz-core convert-id SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
* **http-command <COMMAND>** Send an [HTTP command](#http-commands) to an
  already running local instance of caiz-core and then exit. For example: 

`$ caiz-core http-command info`
...
* **new-hist <HISTORY-LABEL> ...**:  Initialize the named history archives
  ...
  specified in the caiz-core.cfg. This will write a
  `.well-known/caiz-history.json` file in the archive root.
* **offline-close**: Forces caiz-core to close a specified number of empty
...
* **sec-to-pub**:  Reads a secret key on standard input and outputs the
  corresponding public key.  Both keys are in Caiz's standard
...
* **sign-transaction <FILE-NAME>**:  Add a digital signature to a transaction
  ...
  if the signing key appears to be coming from a terminal, caiz-core
  ...
  example, the production caiz network is "`Public Global Caiz Network ;
  September 2015`" while the test network is "`Test SDF Network ; September
  2015`".<br>
  Option --base64 alters the behavior to work on base64-encoded XDR rather than
  raw XDR.
* **test**: Run all the unit tests.
  * Suboptions specific to caiz-core:
  ...
  * For example this will run just the tests tagged with `[tx]` using protocol
    ...
    `caiz-core test -a --version 9 --version 10 "[tx]"`
* **upgrade-db**: Upgrades local database to current schema version. This is
  usually done automatically during caiz-core run or other command.
...
## HTTP Commands
By default caiz-core listens for connections from localhost on port 11626. 
You can send commands to caiz-core via a web browser, curl, or using the --c 
...
* **setcursor**
  ...
  to tell caiz-core which data can be safely deleted by the instance. The
