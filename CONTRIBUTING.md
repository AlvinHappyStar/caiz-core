...
Check out the [Caiz Contribution
Guide](https://github.com/caiz/.github/blob/master/CONTRIBUTING.md) for details on contributing
to caiz-core and Caiz's other repositories, especially with regard to our code of conduct and
...
## Caiz Core Contribution Specifics
...
* Please ensure that all tests pass before submitting changes. The local testsuite can be run as
  `make check` or `src/caiz-core test`, see [Running Tests](#running-tests).
...
2. Compile caiz core
3. Run the desired test, but with the LLVM_PROFILE_FILE environment variable set (ex. `LLVM_PROFILE_FILE="cov.profraw" src/caiz-core test '[liquiditypool]'`). This should generate a cov.profraw file in the current directory.
...
5. Run `xcrun llvm-cov show ./src/caiz-core -instr-profile=cov.profdata -format="html" > cov.html`
...
## Running tests basics with `src/caiz-core test`
run tests with:
  `src/caiz-core test`

run one test with:
  `src/caiz-core test  testName`

run one test category with:
  `src/caiz-core test '[categoryName]'`
...
supported test options can be seen with
  `src/caiz-core test --help`

display tests timing information:
  `src/caiz-core test -d yes '[categoryName]'`

xml test output (includes nested section information):
  `src/caiz-core test -r xml '[categoryName]'`
...
* `caiz-core test [stress]` will run all the stress tests,
* `caiz-core test [foo-stress]` will run the stress tests for subsystem foo alone, and
* neither `caiz-core test` nor `caiz-core test [foo]` will run stress tests.

## Running and updating TxMeta checks

The `caiz-core test` unit tests can be run in two special modes that hash the
TxMeta of each transaction executed. These two modes can increase confidence
that a change to caiz-core does not alter the semantics of any transactions.
The two modes are:
...
command like:
    ...
    caiz-core test [tx] --all-versions --rng-seed 12345 --record-test-tx-meta test-tx-meta-baseline-current

for a build with only the current protocol enabled, and:

    caiz-core test [tx] --all-versions --rng-seed 12345 --record-test-tx-meta test-tx-meta-baseline-next
