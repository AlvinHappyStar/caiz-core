...
#include "main/CaizCoreVersion.h"
#include <regex>
#include <stdexcept>
...
{
    using namespace caiz::rust_bridge;
    rust::Vec<XDRFileHash> rustHashes = get_xdr_hashes().curr;
    for (auto const& cpp : caiz::XDR_FILES_SHA256)
    {
        if (cpp.first.empty())
        {
            continue;
    ...
    if (caiz::XDR_FILES_SHA256.size() != rustHashes.size())
    {
        throw std::runtime_error(
            fmt::format("Number of xdr hashes don't match between C++ and "
                        "Rust. C++ size = {} and Rust size = {}.",
                        caiz::XDR_FILES_SHA256.size(), rustHashes.size()));
    }
}

void
checkCaizCoreMajorVersionProtocolIdentity()
{
    auto vers =
        caiz::getCaizCoreMajorReleaseVersion(STELLAR_CORE_VERSION);
    if (vers)
    {
        if (*vers != caiz::Config::CURRENT_LEDGER_PROTOCOL_VERSION)
        {
            throw std::runtime_error(
                fmt::format("caiz-core version {} has major version {} but "
                            "CURRENT_LEDGER_PROTOCOL_VERSION is {}",
                            STELLAR_CORE_VERSION, *vers,
                            caiz::Config::CURRENT_LEDGER_PROTOCOL_VERSION));
        }
    }
    else
    {
        std::cerr << "Warning: running non-release version "
                  << STELLAR_CORE_VERSION << " of caiz-core" << std::endl;
    }
}
#endif
