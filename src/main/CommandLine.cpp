...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"caiz-core.cfg"} : mConfigFile;
...
runForceSCP(CommandLineArgs const& args)
{
    ...
    auto resetOption = clara::Opt{reset}["--reset"](
        "reset force SCP flag, so next time caiz-core "
        ...
        [&] {
            ...
            try
            {
                ...
                if (cfg.MANUAL_CLOSE)
                {
                    ...
                    {
                        LOG_ERROR(DEFAULT_LOG, "Starting caiz-core in "
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"http-command", "send a command to local caiz-core",
          runHttpCommand},
