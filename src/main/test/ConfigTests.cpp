...
{
    Config c;
    c.load("testdata/caiz-core_example_validators.cfg");
...
TEST_CASE("load example configs", "[config]")
{
    ...
    std::vector<std::string> testFiles = {
        "caiz-core_example.cfg", "caiz-core_standalone.cfg",
        "caiz-core_testnet_legacy.cfg", "caiz-core_testnet.cfg",
        "caiz-core_testnet_validator.cfg"};
...
TEST_CASE("operation filter configuration", "[config]")
{
    ...
    caiz::uniform_int_distribution<size_t> dist(
    ...
    for (size_t i = 0; i < 5; ++i)
    {
        ...
        caiz::shuffle(vals.begin(), vals.end(), gRandomEngine);
