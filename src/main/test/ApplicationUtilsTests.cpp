...
using namespace caiz::historytestutils;
    ...
    damageVictim() override
    {
                      ...
                      /*shouldUpdateLastModified=*/false);
        {
            auto rootKey = accountKey(
                caiz::txtest::getRoot(app->getNetworkID()).getPublicKey());
...
{
    CHECK(getCaizCoreMajorReleaseVersion("v19.0.0") ==
          std::make_optional<uint32_t>(19));
    CHECK(getCaizCoreMajorReleaseVersion("v19.1.3") ==
          std::make_optional<uint32_t>(19));
    CHECK(getCaizCoreMajorReleaseVersion("v19.1.3rc0") ==
          std::make_optional<uint32_t>(19));
    CHECK(getCaizCoreMajorReleaseVersion("v20.0.0rc1") ==
          std::make_optional<uint32_t>(20));
    CHECK(getCaizCoreMajorReleaseVersion("v20.0.0HOT4") ==
          std::make_optional<uint32_t>(20));
    CHECK(getCaizCoreMajorReleaseVersion("v19.1.2-10") == std::nullopt);
    CHECK(getCaizCoreMajorReleaseVersion("v19.9.0-30-g726eabdea-dirty") ==
          std::nullopt);
}