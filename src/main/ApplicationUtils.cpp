         ...
         std::string const& startAtHash)
{
    ...
    LOG_INFO(DEFAULT_LOG, "Starting caiz-core {}", STELLAR_CORE_VERSION);
    Application::pointer app;
...
closeLedgersOffline(Config cfg, bool verbose, size_t nLedgers)
{
    ...
    while (!app->isStopping() && lclSeq < targetSeq)
    {
        ...
        auto sv = app->getHerder().makeCaizValue(
        ...
        Json::Value& catchupInfo, std::shared_ptr<HistoryArchive> archive)
{
    ...
    catch (std::invalid_argument const&)
    {
        ...
        LOG_INFO(
            DEFAULT_LOG,
            "* If you really want to catchup to {} run caiz-core new-db",
            cc.toLedger());
        LOG_INFO(DEFAULT_LOG, "*");
        return 2;
    }
...
// release-tagged version of caiz core (one that looks like vNN.X.Y or
// vNN.X.YrcZ or vNN.X.YHOTZ). If its version has some other name structure
// structure, return std::nullopt.
std::optional<uint32_t>
getCaizCoreMajorReleaseVersion(std::string const& vstr)
{
