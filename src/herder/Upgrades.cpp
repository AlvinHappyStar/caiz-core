...
void
save(Archive& ar, caiz::Upgrades::UpgradeParameters const& p)
{
    ...
    ar(make_nvp("time", caiz::VirtualClock::to_time_t(p.mUpgradeTime)));
    ...
    {
        configUpgradeKeyStr = caiz::decoder::encode_b64(
...
template <class Archive>
void
load(Archive& ar, caiz::Upgrades::UpgradeParameters& o,
     caiz::AbstractLedgerTxn& ltx)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = caiz::VirtualClock::from_time_t(t);
    ...
    try
    {
        ...
        if (configUpgradeKeyStr)
        {
            ...
            caiz::decoder::decode_b64(configUpgradeKeyStr.value(), buffer);
            caiz::ConfigUpgradeSetKey key;
...
std::string
Upgrades::UpgradeParameters::toDebugJson(caiz::AbstractLedgerTxn& ltx) const
{
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!caiz::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerTxn& ltx)
{
    ...
    {
        auto trust = caiz::loadTrustLineWithoutRecord(ltx, accountID, asset);
    ...
    std::map<AccountID, std::vector<LedgerTxnEntry>> const& offersByAccount)
{
    ...
    for (auto const& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = caiz::loadAccount(ltx, accountID);
        if (!accountEntry)
        {
...
prepareLiabilities(AbstractLedgerTxn& ltx, LedgerTxnHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = caiz::loadAccount(ltx, accountOffers.first);
        if (!accountEntry)
        {
            throw std::runtime_error("account does not exist");
        }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    caiz::loadTrustLine(ltx, accountOffers.first, asset);
                int64_t deltaSelling =
...
upgradeFromProtocol15To16(AbstractLedgerTxn& ltx)
{
    if (gIsProductionNetwork)
    {
        ...
        auto seller = caiz::loadAccountWithoutRecord(ltx, sellerID);
        auto offer = caiz::loadOffer(ltx, sellerID, offerID);
