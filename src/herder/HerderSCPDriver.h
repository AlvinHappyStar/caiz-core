...
class HerderSCPDriver : public SCPDriver
{
  public:
    ...
    void nominate(uint64_t slotIndex, CaizValue const& value,
    ...
    // converts a Value into a CaizValue
    // returns false on error
    bool toCaizValue(Value const& v, CaizValue& sv);
    ...
    // wraps a *valid* CaizValue (throws if it can't find txSet/qSet)
    ValueWrapperPtr wrapCaizValue(CaizValue const& sv);
