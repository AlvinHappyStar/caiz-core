...
HerderSCPDriver::validateValueHelper(uint64_t slotIndex, CaizValue const& b,
                                     bool nomination) const
{
...

    {
        ...
        if (!mHerder.verifyCaizValueSignature(b))
        {
            return SCPDriver::kInvalidValue;
        }
    }
...
HerderSCPDriver::getValueString(Value const& v) const
{
    ...
    try
    {
        ...
        return caizValueToString(mApp.getConfig(), b);
    }
    catch (...)
    {
        return "[:invalid:]";
                                   ...
                                   ValueWrapperPtrSet const& candidates)
{
    ...
    std::vector<CaizValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        CaizValue& sv = candidateValues.back();
    ...
    auto res = wrapCaizValue(comp);
    return res;
}

bool
HerderSCPDriver::toCaizValue(Value const& v, CaizValue& sv)
{
    try
    {
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid CaizValue.
        CLOG_ERROR(Herder, "HerderSCPDriver::valueExternalized "
                           "Externalized CaizValue malformed");
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, CaizValue const& value,
                          TxSetFrameConstPtr proposedSet,
                          CaizValue const& previousValue)
{
    ZoneScoped;
    mCurrentValue = wrapCaizValue(value);
...
class SCPHerderValueWrapper : public ValueWrapper
{
  ...
  public:
    explicit SCPHerderValueWrapper(CaizValue const& sv, Value const& value,
                                   HerderImpl& herder)
...
{
    CaizValue sv;
    auto b = mHerder.getHerderSCPDriver().toCaizValue(val, sv);
    if (!b)
    {
        throw std::runtime_error(
...
ValueWrapperPtr
HerderSCPDriver::wrapCaizValue(CaizValue const& sv)
{
