...
class HerderImpl : public Herder
{
  public:
    ...
    void setTrackingSCPState(uint64_t index, CaizValue const& value,
    ...
    void processExternalized(uint64 slotIndex, CaizValue const& value);
    void valueExternalized(uint64 slotIndex, CaizValue const& value,
                           bool isLatestSlot);
    ...
    virtual CaizValue
    makeCaizValue(Hash const& txSetHash, uint64_t closeTime,
                     xdr::xvector<UpgradeType, 6> const& upgrades,
    ...
    bool verifyCaizValueSignature(CaizValue const& sv);
  ...
  private:
    ...
    void newSlotExternalized(bool synchronous, CaizValue const& value);
