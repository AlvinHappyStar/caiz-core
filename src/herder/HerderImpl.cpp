...
void
HerderImpl::setTrackingSCPState(uint64_t index, CaizValue const& value,
...
void
HerderImpl::newSlotExternalized(bool synchronous, CaizValue const& value)
{
    ZoneScoped;
...
void
HerderImpl::processExternalized(uint64 slotIndex, CaizValue const& value)
{
    ZoneScoped;
...
void
HerderImpl::valueExternalized(uint64 slotIndex, CaizValue const& value,
                              bool isLatestSlot)
{
    ZoneScoped;
                             ...
                             std::optional<SecretKey> skToSignValue)
{
    ...
    CaizValue sv =
        makeCaizValue(txSet->getContentsHash(), closeTime, upgrades, sk);
...
HerderImpl::sendSCPStateToPeer(uint32 ledgerSeq, Peer::pointer peer)
{
    ...
    auto sendSlot = [weakPeer = std::weak_ptr<Peer>(peer)](SCPEnvelope const& e,
        ...
        m.type(SCP_MESSAGE);
        m.envelope() = e;
        auto mPtr = std::make_shared<CaizMessage const>(m);
                              ...
                              bool checkTrackingSCP)
{
    ...
    // the exact closeTime contained in the CaizValue with the best
    ...
    CaizValue newProposedValue = makeCaizValue(
...
bool
HerderImpl::verifyCaizValueSignature(CaizValue const& sv)
{
    ZoneScoped;
    auto b = PubKeyUtils::verifySig(
...
CaizValue
HerderImpl::makeCaizValue(Hash const& txSetHash, uint64_t closeTime,
                             xdr::xvector<UpgradeType, 6> const& upgrades,
                             SecretKey const& s)
{
    ZoneScoped;
