...
using namespace caiz;
using namespace caiz::txbridge;
using namespace caiz::txtest;
...
makeMultiPayment(caiz::TestAccount& destAccount, caiz::TestAccount& src,
                 int nbOps, int64 paymentBase, uint32 extraFee, uint32 feeMult)
{
    std::vector<caiz::Operation> ops;
...
static TransactionFramePtr
makeSelfPayment(caiz::TestAccount& account, int nbOps, uint32_t fee)
{
    std::vector<caiz::Operation> ops;
    for (int i = 0; i < nbOps; i++)
    {
...
{
    using SVUpgrades = decltype(CaizValue::upgrades);
    ...
    auto makeTxUpgradePair = [&](HerderImpl& herder, TxSetFrameConstPtr txSet,
        ...
        CaizValue sv = herder.makeCaizValue(
    ...
    SECTION("combineCandidates")
    {
        ...
        auto addCandidateThenTest = [&](CandidateSpec const& spec) {
            ...
            // increment, and make it into a CaizValue to add to the list
            ...
            // Compare the returned CaizValue's contents with the
    ...
    SECTION("validateValue signatures")
    {
        ...
        SECTION("invalid")
        {
            auto checkInvalid = [&](CaizValue const& sv, bool nomination) {
                auto v = xdr::xdr_to_opaque(sv);
            ...
            auto testInvalidValue = [&](bool isNomination) {
                ...
                {
                    auto basicVal =
                        CaizValue(txSet0->getContentsHash(), ct,
    ...
    SECTION("validateValue closeTimes")
    {
        ...
        auto testTxBounds = [&](TimePoint const minTime,
            ...
            // Build a CaizValue containing the transaction set we just
...
TEST_CASE("tx queue source account limit", "[herder][transactionqueue]")
{
    ...
    SECTION("mixed")
    {
        ...
        for (auto const& node : simulation->getNodes())
        {
            ...
            auto acc1 = caiz::loadAccount(ltx, a1.getPublicKey());
            auto acc2 = caiz::loadAccount(ltx, b1.getPublicKey());
            REQUIRE(acc1);
            REQUIRE(acc2);
    ...
    SECTION("all limited")
    {
        ...
        for (auto const& node : simulation->getNodes())
        {
            ...
            LedgerTxn ltx(node->getLedgerTxnRoot());
            REQUIRE(caiz::loadAccount(ltx, a1.getPublicKey()));
            REQUIRE(!caiz::loadAccount(ltx, b1.getPublicKey()));
        }
    ...
    for (auto start = currentLedger + 1; start <= destinationLedger; start++)
    {
        for (auto const& env : herderA.getSCP().getLatestMessagesSend(start))
        {
            if (env.statement.pledges.type() == SCP_ST_EXTERNALIZE)
            {
                ...
                herderA.getHerderSCPDriver().toCaizValue(
        ...
        for (auto const& env : herderB.getSCP().getLatestMessagesSend(start))
        {
            if (env.statement.pledges.type() == SCP_ST_EXTERNALIZE)
            {
                ...
                herderB.getHerderSCPDriver().toCaizValue(
                    env.statement.pledges.externalize().commit.value, sv);
                auto txset = pe.getTxSet(sv.txSetHash);
                REQUIRE(txset);
            ...
            std::vector<TransactionFrameBasePtr> const& txs, Application& app)
{
    ...
    CaizValue sv =
        herder.makeCaizValue(txSet->getContentsHash(), lastCloseTime,
