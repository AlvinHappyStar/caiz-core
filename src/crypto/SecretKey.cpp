...
SecretKey::pseudoRandomForTestingFromSeed(unsigned int seed)
{
    ...
    // to guess PRNG-ness. It's intended for _deterministic_ use, when you want
    // "slightly random-ish" keys, for test-data generation.
    caiz_default_random_engine tmpEngine(seed);
...
{
size_t
hash<caiz::PublicKey>::operator()(caiz::PublicKey const& k) const noexcept
{
    using namespace caiz;
    releaseAssert(k.type() == caiz::PUBLIC_KEY_TYPE_ED25519);

    return std::hash<caiz::uint256>()(k.ed25519());
}
}
