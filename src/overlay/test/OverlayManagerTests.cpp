...
class FlowControlStub : public FlowControl
{
  public:
    ...
    virtual bool
    maybeSendMessage(std::shared_ptr<CaizMessage const> msg) override
    {
        // mock flow control
        mSent++;
        return true;
...
class OverlayManagerTests
{
  ...
  protected:
    ...
    testBroadcast()
    {
        ...
        CaizMessage AtoB = a.tx({payment(b, 10)})->toCaizMessage();
        auto i = 0;
        for (auto p : pm.mOutboundPeers.mAuthenticated)
        {
            if (i++ == 2)
        ...
        crank(10);
        REQUIRE(sentCounts(pm) == expected);
        CaizMessage CtoD = c.tx({payment(d, 10)})->toCaizMessage();
