...
using namespace caiz::overlaytestutils;
    ...
    auto test = [&](bool shouldRequestMore) {
        ...
        SECTION("basic capacity accounting")
        {
            ...
            conn.getInitiator()->sendMessage(
                std::make_shared<CaizMessage>(msg));
...
TEST_CASE("drop idle flow-controlled peers", "[overlay][flowcontrol]")
{
    ...
    auto test = [&](bool fcBytes) {
        ...
        conn.getAcceptor()->sendMessage(std::make_shared<CaizMessage>(msg),
                                        false);
        conn.getAcceptor()->sendMessage(std::make_shared<CaizMessage>(msg),
                                        false);
...
TEST_CASE("outbound queue filtering", "[overlay][connections]")
{
    ...
    auto constructSCPMsg = [&](SCPEnvelope const& env) {
        ...
        msg.envelope() = env;
        return std::make_shared<CaizMessage const>(msg);
    };
    ...
    SECTION("advert demand limit reached")
    {
        SECTION("count-based")
        {
            ...
            for (uint32_t i = 0; i < limit + 10; ++i)
            {
                ...
                peer->getFlowControl()->addToQueueAndMaybeTrimForTesting(
                    std::make_shared<CaizMessage const>(adv));
                peer->getFlowControl()->addToQueueAndMaybeTrimForTesting(
                    std::make_shared<CaizMessage const>(dem));
            }
...
TEST_CASE("overlay pull mode", "[overlay][pullmode]")
{
    ...
    auto createTxn = [](auto n) {
        ...
        return std::make_shared<CaizMessage>(txn);
    };
    ...
    auto createAdvert = [](auto txns) {
        ...
        return std::make_shared<CaizMessage>(adv);
    };
    ...
    SECTION("ignore duplicated adverts")
    {
        auto tx = createTxn(0);
        auto adv =
            createAdvert(std::vector<std::shared_ptr<CaizMessage>>{tx});
    ...
    SECTION("do not advertise to peers that know about tx")
    {
        ...
        auto adv = createAdvert(std::vector<std::shared_ptr<CaizMessage>>{
    ...
    SECTION("sanity check - demand")
    {
        ...
        auto adv0 =
            createAdvert(std::vector<std::shared_ptr<CaizMessage>>{tx0});
        auto adv1 =
            createAdvert(std::vector<std::shared_ptr<CaizMessage>>{tx1});
    ...
    {
        std::vector<std::shared_ptr<CaizMessage>> txns;
    ...
    SECTION("overlapping adverts")
    {
        ...
        auto adv0 = createAdvert(
            std::vector<std::shared_ptr<CaizMessage>>{tx0, tx1, tx3});
        auto adv1 = createAdvert(
            std::vector<std::shared_ptr<CaizMessage>>{tx0, tx2, tx3});
          ...
          "[overlay][pullmode][acceptance]")
{
    ...
    for (auto& conn : connections)
    {
        ...
        conn->getInitiator()->sendMessage(
            std::make_shared<CaizMessage>(adv));
    }
