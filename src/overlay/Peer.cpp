...
Peer::sendHello()
{
    ...
    auto msgPtr = std::make_shared<CaizMessage const>(msg);
    sendMessage(msgPtr);
}

void
Peer::beginMessageProcessing(CaizMessage const& msg)
{
...
void
Peer::endMessageProcessing(CaizMessage const& msg)
{
    if (shouldAbort())
    {
        return;
...
Peer::sendGetTxSet(uint256 const& setID)
{
    ...
    auto msgPtr = std::make_shared<CaizMessage const>(newMsg);
    sendMessage(msgPtr);
}
...
Peer::sendError(ErrorCode error, std::string const& message)
{
    ...
    m.error().code = error;
    m.error().msg = message;
    auto msgPtr = std::make_shared<CaizMessage const>(m);
    sendMessage(msgPtr);
}
...
std::string
Peer::msgSummary(CaizMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(std::shared_ptr<CaizMessage const> msg, bool log)
{
    ZoneScoped;
...
void
Peer::sendAuthenticatedMessage(CaizMessage const& msg)
{
...
void
Peer::recvMessage(CaizMessage const& caizMsg)
{
    ZoneScoped;
    if (shouldAbort())
    {
    ...
    auto msgTracker = std::make_shared<MsgCapacityTracker>(weak, caizMsg);
...
void
Peer::recvSendMore(CaizMessage const& msg)
{
    releaseAssert(mFlowControl);
...
void
Peer::recvRawMessage(CaizMessage const& caizMsg)
{
    ZoneScoped;
    ...
    if (!isAuthenticated() && (caizMsg.type() != HELLO) &&
        (caizMsg.type() != AUTH) && (caizMsg.type() != ERROR_MSG))
    {
    ...
    releaseAssert(isAuthenticated() || caizMsg.type() == HELLO ||
                  caizMsg.type() == AUTH || caizMsg.type() == ERROR_MSG);
    mApp.getOverlayManager().recordMessageMetric(caizMsg,
                                                 shared_from_this());
    ...
    case SURVEY_REQUEST:
    {
        ...
        recvSurveyRequestMessage(caizMsg);
    }
    break;
    ...
    case SURVEY_RESPONSE:
    {
        ...
        recvSurveyResponseMessage(caizMsg);
    }
    break;
    ...
    case SEND_MORE_EXTENDED:
    {
        std::string errorMsg;
        releaseAssert(mFlowControl);
        if (!mFlowControl->isSendMoreValid(caizMsg, errorMsg))
        {
    ...
    case FLOOD_DEMAND:
    {
        ...
        recvFloodDemand(caizMsg);
    }
    }
}

void
Peer::recvDontHave(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetTxSet(CaizMessage const& msg)
{
    ZoneScoped;
    auto self = shared_from_this();
    if (auto txSet = mApp.getHerder().getTxSet(msg.txSetHash()))
    {
        ...
        auto newMsgPtr = std::make_shared<CaizMessage const>(newMsg);
...
void
Peer::recvTxSet(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGeneralizedTxSet(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvTransaction(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetSCPQuorumSet(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSCPQuorumSet(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSCPMessage(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetSCPState(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvError(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvAuth(CaizMessage const& msg)
{
    ZoneScoped;
    if (mState != GOT_HELLO)
    {
    ...
    auto sendCb = [weakSelf](CaizMessage const& msg) {
...
void
Peer::recvGetPeers(CaizMessage const& msg)
{
    ZoneScoped;
    sendPeers();
}
...
void
Peer::recvPeers(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSurveyRequestMessage(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSurveyResponseMessage(CaizMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvFloodAdvert(CaizMessage const& msg)
{
...
void
Peer::recvFloodDemand(CaizMessage const& msg)
{
...
Peer::sendTxDemand(TxDemandVector&& demands)
{
    if (demands.size() > 0)
    {
        auto msg = std::make_shared<CaizMessage>();
...
Peer::flushAdvert()
{
    if (mTxHashesToAdvertise.size() > 0)
    {
        ...
        auto msg = std::make_shared<CaizMessage>(adv);
        std::weak_ptr<Peer> weak(static_pointer_cast<Peer>(shared_from_this()));
        mApp.postOnMainThread(
            [weak, msg = std::move(msg)]() {
                auto strong = weak.lock();
...
Peer::fulfillDemand(FloodDemand const& dmd)
{
    ...
    for (auto const& h : dmd.txHashes)
    {
        ...
        if (tx)
        {
            ...
            auto smsg =
                std::make_shared<CaizMessage>(tx->toCaizMessage());
            sendMessage(smsg);
        }
        else
        {
