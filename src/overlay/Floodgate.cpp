...
bool
Floodgate::addRecord(CaizMessage const& msg, Peer::pointer peer, Hash& index)
{
    ZoneScoped;
    index = xdrBlake2(msg);
    if (mShuttingDown)
...
bool
Floodgate::broadcast(CaizMessage const& msg, bool force,
                     std::optional<Hash> const& hash)
{
    ...
    auto smsg = std::make_shared<CaizMessage const>(msg);
    for (auto peer : peers)
    {
