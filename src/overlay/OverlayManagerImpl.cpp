...
bool
OverlayManagerImpl::isFloodMessage(CaizMessage const& msg)
{
...
{
    caiz::shuffle(peerList.begin(), peerList.end(), gRandomEngine);
}

bool
OverlayManagerImpl::recvFloodedMsgID(CaizMessage const& msg,
...
bool
OverlayManagerImpl::broadcastMessage(CaizMessage const& msg, bool force,
...
void
OverlayManagerImpl::recordMessageMetric(CaizMessage const& caizMsg,
                                        Peer::pointer peer)
{
    ZoneScoped;
    auto logMessage = [&](bool unique, std::string const& msgType) {
        CLOG_TRACE(Overlay, "recv: {} {} ({}) of size: {} from: {}",
                   ...
                   peer->msgSummary(caizMsg), msgType,
                   xdr::xdr_argpack_size(caizMsg),
                   mApp.getConfig().toShortString(peer->getPeerID()));
    };

    bool flood = false;
    if (isFloodMessage(caizMsg) || caizMsg.type() == SURVEY_REQUEST ||
        caizMsg.type() == SURVEY_RESPONSE)
    {
        flood = true;
    }
    else if (caizMsg.type() != TX_SET && caizMsg.type() != SCP_QUORUMSET)
    {
        return;
    }
    ...
    size_t size = xdr::xdr_argpack_size(caizMsg);
    auto hash = shortHash::xdrComputeHash(caizMsg);
