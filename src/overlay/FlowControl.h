...
class FlowControl
{
  public:
    ...
    {
        std::shared_ptr<CaizMessage const> mMessage;
  ...
  private:
    ...
    std::function<void(CaizMessage const&)> mSendCallback;
    ...
    void addMsgAndMaybeTrimQueue(std::shared_ptr<CaizMessage const> msg);
    ...
    bool hasOutboundCapacity(CaizMessage const& msg) const;

  public:
    FlowControl(Application& app);
    virtual ~FlowControl() = default;

    virtual bool maybeSendMessage(std::shared_ptr<CaizMessage const> msg);
    void maybeReleaseCapacityAndTriggerSend(CaizMessage const& msg);
    ...
    void
    addToQueueAndMaybeTrimForTesting(std::shared_ptr<CaizMessage const> msg)
    {
    ...
    static uint32_t getNumMessages(CaizMessage const& msg);
    bool isSendMoreValid(CaizMessage const& msg,
                         std::string& errorMsg) const;

    // This method ensures local capacity is locked now that we've received a
    // new message
    bool beginMessageProcessing(CaizMessage const& msg);
    ...
    void start(std::weak_ptr<Peer> peer,
               std::function<void(CaizMessage const&)> sendCb,
               bool enableFCBytes);
};
