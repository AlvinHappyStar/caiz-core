                      ...
                      public NonMovableOrCopyable
{
  public:
    ...
    void relayOrProcessResponse(CaizMessage const& msg, Peer::pointer peer);
    void relayOrProcessRequest(CaizMessage const& msg, Peer::pointer peer);
    void clearOldLedgers(uint32_t lastClosedledgerSeq);
    Json::Value const& getJsonResults();

    static std::string getMsgSummary(CaizMessage const& msg);
    CaizMessage makeSurveyRequest(NodeID const& nodeToSurvey) const;
  ...
  private:
    ...
    void broadcast(CaizMessage const& msg) const;
