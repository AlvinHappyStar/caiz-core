...
class FlowControlCapacity
{
  ...
  public:
    virtual uint64_t getMsgResourceCount(CaizMessage const& msg) const = 0;
    virtual ReadingCapacity getCapacityLimits() const = 0;
    virtual void releaseOutboundCapacity(CaizMessage const& msg) = 0;

    void lockOutboundCapacity(CaizMessage const& msg);
    bool lockLocalCapacity(CaizMessage const& msg);
    // Release capacity used by this message. Return how flood capacity was
    // freed
    uint64_t releaseLocalCapacity(CaizMessage const& msg);
...
class FlowControlByteCapacity : public FlowControlCapacity
{
  ...
  public:
    ...
    virtual uint64_t
    getMsgResourceCount(CaizMessage const& msg) const override;
    virtual ReadingCapacity getCapacityLimits() const override;
    virtual void releaseOutboundCapacity(CaizMessage const& msg) override;
...
class FlowControlMessageCapacity : public FlowControlCapacity
{
  public:
    ...
    getMsgResourceCount(CaizMessage const& msg) const override;
    virtual ReadingCapacity getCapacityLimits() const override;
    void releaseOutboundCapacity(CaizMessage const& msg) override;
    bool canRead() const override;
};
}