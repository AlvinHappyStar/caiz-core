...
{
#ifdef ENABLE_NEXT_PROTOCOL_VERSION_UNSAFE_FOR_PRODUCTION
    // We need to keep track of lumens in the Caiz Asset Contract, so
...
#ifdef ENABLE_NEXT_PROTOCOL_VERSION_UNSAFE_FOR_PRODUCTION
            return lhs + caiz::calculateDeltaBalance(
                             mLumenContractID, mBalanceSymbol, mAmountSymbol,
                             rhs.second.current, rhs.second.previous);
#else
            return lhs + caiz::calculateDeltaBalance(rhs.second.current,
                                                        rhs.second.previous);
#endif
        });
