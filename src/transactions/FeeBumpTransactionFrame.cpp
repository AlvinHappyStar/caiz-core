...
FeeBumpTransactionFrame::commonValidPreSeqNum(AbstractLedgerTxn& ltx)
{
    ...
    if (!caiz::loadAccount(ltx, getFeeSourceID()))
    {
                                     ...
                                     AbstractLedgerTxn& ltxOuter, bool applying)
{
    ...
    auto feeSource = caiz::loadAccount(ltx, getFeeSourceID());
    if (!checkSignature(
...
{
    auto account = caiz::loadAccount(ltx, getFeeSourceID());
    if (!account)
    {
...
CaizMessage
FeeBumpTransactionFrame::toCaizMessage() const
{
