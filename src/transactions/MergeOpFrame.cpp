...
MergeOpFrame::doApplyBeforeV16(AbstractLedgerTxn& ltx)
{
    ...
    auto otherAccount =
        caiz::loadAccount(ltx, toAccountID(mOperation.body.destination()));
    if (!otherAccount)
    {
...
MergeOpFrame::doApplyFromV16(AbstractLedgerTxn& ltx)
{
    ...
    if (!caiz::loadAccount(ltx, toAccountID(mOperation.body.destination())))
    {
        innerResult().code(ACCOUNT_MERGE_NO_ACCOUNT);
        return false;
    }
    ...
    {
        auto otherAccount = caiz::loadAccount(
