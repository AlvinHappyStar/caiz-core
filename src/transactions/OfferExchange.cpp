           ...
           std::vector<ClaimAtom>& offerTrail)
{
    ...
    {
        auto accountB = caiz::loadAccountWithoutRecord(ltx, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        auto accountB = caiz::loadAccount(ltx, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = caiz::loadAccount(ltxInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                caiz::loadTrustLine(ltxInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                caiz::loadTrustLine(ltxInner, accountBID, wheat);
              ...
              RoundingType round, std::vector<ClaimAtom>& offerTrail)
{
    ...
    if (!caiz::loadAccountWithoutRecord(ltx, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = caiz::loadAccount(ltx, accountBID);
    }
