                            ...
                            LedgerTxnEntry const& offerEntry, bool isAcquire)
{
    ...
    auto loadAccountAndValidate = [&ltx, &sellerID]() {
        auto account = caiz::loadAccount(ltx, sellerID);
        if (!account)
        {
    ...
    auto loadTrustAndValidate = [&ltx, &sellerID](Asset const& asset) {
        auto trust = caiz::loadTrustLine(ltx, sellerID, asset);
        if (!trust)
        {
                            ...
                            LedgerTxnEntry& entry, int64_t amount)
{
    ...
    if (!caiz::addBalance(newBalance, amount, tl.limit))
    {
        return false;
    }
...
addBalance(LedgerTxnHeader const& header, LedgerTxnEntry& entry, int64_t delta)
{
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        if (!caiz::addBalance(newBalance, delta))
        {
            return false;
        }
        if (protocolVersionStartsFrom(header.current().ledgerVersion,
                     ...
                     int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = caiz::addBalance(buyingLiab, delta, maxLiabilities);
        if (res)
        {
                      ...
                      int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = caiz::addBalance(sellingLiab, delta, maxLiabilities);
        if (res)
        {
                              ...
                              Asset const& asset)
{
    ...
    for (auto& offer : offers)
    {
        ...
        auto trustAcc = caiz::loadAccount(ltxInner, account);
                                   ...
                                   SequenceNumber txSeqNum, uint32_t opIndex)
{
    ...
    for (auto const& poolTLKey : poolTLKeys)
    {
        ...
        {
            auto trustAcc = caiz::loadAccount(ltxInner, accountID);
        ...
        auto redeemIntoClaimableBalance =
            ...
            if (loadSponsorship(ltxInner, cbSponsoringAccID))
            {
                auto cbSponsoringLtxAcc =
                    caiz::loadAccount(ltxInner, cbSponsoringAccID);
...
struct MuxChecker
{
    ...
    void
    operator()(caiz::MuxedAccount const& t)
    {
