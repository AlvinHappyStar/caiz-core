                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = caiz::loadAccount(ltx, accountID);
        if (res)
        {
    ...
    {
        return caiz::loadAccount(ltx, accountID);
    }
}
                                   ...
                                   AbstractLedgerTxn& ltxOuter)
{
    ...
    caiz::addBalance(acc.balance, feeRefund);
                                       ...
                                       uint64_t upperBoundCloseTimeOffset)
{
    ...
    if (isSoroban())
    {
        ...
        auto checkDuplicates =
            [&](xdr::xvector<caiz::LedgerKey> const& keys) -> bool {
                                   ...
                                   std::optional<int64_t> baseFee)
{
    ...
    if (fee > 0)
    {
        ...
        caiz::addBalance(acc.balance, -fee);
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    auto account = caiz::loadAccount(ltx, accountID);
    if (!account)
    {
...
CaizMessage
TransactionFrame::toCaizMessage() const
{
    CaizMessage msg(TRANSACTION);
