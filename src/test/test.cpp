    ...
    if (!cfgs[instanceNumber])
    {
        if (gTestRoots.empty())
        {
            gTestRoots.emplace_back(
                fmt::format("caiz-core-test-{}", gBaseInstance));
        }
...
runTest(CommandLineArgs const& args)
{
    ...
    parser |= Catch::clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "caiz-core can run tests concurrently");
    parser |=
    ...
    // not mixed with caiz-core logging.
    Logging::setFmt("<test>");
    Logging::setLogLevel(logLevel, nullptr);
    // use base instance for logging as we're guaranteed to not have conflicting
    // instances of caiz-core running at the same time with the same base
    // instance
    auto logFile = fmt::format("caiz{}.log", gBaseInstance);
    Logging::setLoggingToFile(logFile);
    Logging::setLogLevel(logLevel, nullptr);

    LOG_INFO(DEFAULT_LOG, "Testing caiz-core {}", STELLAR_CORE_VERSION);
