...
struct xdr_fuzzer_compactor
{
    ...
    typename std::enable_if<
        (!std::is_same<caiz::AccountID, T>::value &&
         !std::is_same<caiz::MuxedAccount, T>::value &&
         !std::is_same<caiz::Asset, T>::value &&
         !std::is_same<caiz::AssetCode, T>::value &&
         !std::is_same<caiz::ClaimableBalanceID, T>::value &&
         !std::is_same<caiz::LedgerKey, T>::value) &&
    ...
    template <typename T>
    typename std::enable_if<std::is_same<caiz::AccountID, T>::value>::type
    operator()(T const& pk)
    {
        // convert public key 1 byte
        check(1);
        auto b = caiz::FuzzUtils::getShortKey(pk.ed25519());
        put_bytes(&b, 1);
    }
    template <typename T>
    typename std::enable_if<std::is_same<caiz::MuxedAccount, T>::value>::type
    operator()(T const& m)
    {
        // convert MuxedAccount -> 1 byte (same than an AccountID)
        auto const& ed25519 = (m.type() == caiz::KEY_TYPE_ED25519)
                                  ? m.ed25519()
                                  : m.med25519().ed25519;
        check(1);
        auto b = caiz::FuzzUtils::getShortKey(ed25519);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<caiz::Asset, T>::value>::type
    operator()(T const& asset)
    {
        // Convert Asset to 1 byte.
        check(1);
        auto b = caiz::FuzzUtils::getShortKey(asset);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<caiz::AssetCode, T>::value>::type
    operator()(T const& code)
    {
        // Convert AssetCode to 1 byte.
        check(1);
        auto b = caiz::FuzzUtils::getShortKey(code);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<caiz::ClaimableBalanceID, T>::value>::type
    operator()(T const& balanceID)
    {
        ...
        check(1);
        auto b = caiz::FuzzUtils::getShortKey(balanceID);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<caiz::LedgerKey, T>::value>::type
    operator()(T const& key)
    {
        ...
        check(1);
        auto b = caiz::FuzzUtils::getShortKey(key);
        put_bytes(&b, 1);
    }
};
...
{
    caiz::FuzzUtils::StoredLedgerKeys mStoredLedgerKeys;
    caiz::FuzzUtils::StoredPoolIDs mStoredPoolIDs;
    std::uint8_t const* mCur;
    std::uint8_t const* const mEnd;

    xdr_fuzzer_unpacker(
        caiz::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
        caiz::FuzzUtils::StoredPoolIDs const& storedPoolIDs,
    ...
    typename std::enable_if<
        (!std::is_same<caiz::AccountID, T>::value &&
         ...
         !std::is_same<caiz::AssetCode, T>::value &&
         !std::is_same<caiz::ClaimableBalanceID, T>::value &&
         !std::is_same<caiz::LiquidityPoolDepositOp, T>::value &&
         !std::is_same<caiz::LiquidityPoolWithdrawOp, T>::value &&
         !std::is_same<caiz::LedgerKey, T>::value) &&
        (xdr_traits<T>::is_class || xdr_traits<T>::is_container)>::type
    operator()(T& t)
    {
    ...
    operator()(T& pk)
    {
        ...
        caiz::FuzzUtils::setShortKey(pk, v);
    }
    template <typename T>
    typename std::enable_if<std::is_same<caiz::MuxedAccount, T>::value>::type
    operator()(T& m)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        caiz::FuzzUtils::setShortKey(m.ed25519(), v);
    }
    ...
    operator()(T& asset)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        asset = caiz::FuzzUtils::makeAsset(v);
    }
    ...
    operator()(T& code)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        code = caiz::FuzzUtils::makeAssetCode(v);
    }
    ...
    operator()(T& balanceID)
    {
        ...
        std::uint8_t v = get_byte();
        caiz::LedgerKey key;
        caiz::FuzzUtils::setShortKey(mStoredLedgerKeys, key, v);
        ...
        if (key.type() == caiz::CLAIMABLE_BALANCE)
        {
            balanceID = key.claimableBalance().balanceID;
        }
        else
        {
            balanceID.type(caiz::CLAIMABLE_BALANCE_ID_TYPE_V0);
            balanceID.v0()[0] = v;
        }
    }
    ...
    template <typename T>
    typename std::enable_if<
        std::is_same<caiz::LiquidityPoolDepositOp, T>::value>::type
    operator()(T& depositOp)
    {
        check(1);
        auto v = get_byte();
        caiz::FuzzUtils::setShortKey(mStoredPoolIDs,
        ...
        depositOp.minPrice = caiz::Price{minN, minD};
        depositOp.maxPrice = caiz::Price{maxN, maxD};
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<caiz::LiquidityPoolWithdrawOp, T>::value>::type
...
xdr_from_fuzzer_opaque(
    caiz::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
    caiz::FuzzUtils::StoredPoolIDs const& storedPoolIDs, Bytes const& m,
...
template <>
void
generator_t::operator()(caiz::PublicKey& t) const
{
    ...
    static_assert(NUMBER_OF_ACCOUNT_IDS_TO_GENERATE >
                      caiz::FuzzUtils::NUMBER_OF_PREGENERATED_ACCOUNTS,
                  "Range of generated accounts too small");
    caiz::FuzzUtils::setShortKey(
        t.ed25519(), static_cast<uint8_t>(caiz::rand_uniform<int>(
...
bool
isBadOverlayFuzzerInput(CaizMessage const& m)
{
...
TransactionFuzzer::storeSetupLedgerKeysAndPoolIDs(AbstractLedgerTxn& ltx)
{
    ...
    caiz::FuzzUtils::generateStoredLedgerKeys(firstGeneratedLedgerKey,
...
TransactionFuzzer::reduceNativeBalancesAfterSetup(AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& param : accountParameters)
    {
        ...
        auto ae = caiz::loadAccount(ltx, account);
    ...
    AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& trustLine : trustLineParameters)
    {
        ...
        // issuer.
        auto tle = caiz::loadTrustLine(ltx, account, asset);
...
{
    reinitializeAllGlobalStateWithSeed(1);
    caiz::FuzzUtils::generateStoredLedgerKeys(mStoredLedgerKeys.begin(),
                                                 mStoredLedgerKeys.end());
    ...
    initiator->getApp().getClock().postAction(
        [initiator, msg]() {
            initiator->Peer::sendMessage(
                std::make_shared<CaizMessage const>(msg));
        },
...
OverlayFuzzer::genFuzz(std::string const& filename)
{
    ...
    out.exceptions(std::ios::failbit | std::ios::badbit);
    out.open(filename, std::ofstream::binary | std::ofstream::trunc);
    autocheck::generator<CaizMessage> gen;
    CaizMessage m(gen(FUZZER_INITIAL_CORPUS_MESSAGE_GEN_UPPERBOUND));
