...
TestAccount::updateSequenceNumber()
{
    if (mSn == 0)
    {
        ...
        auto entry = caiz::loadAccount(ltx, getPublicKey());
        if (entry)
        {
...
{
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto trustLine = caiz::loadTrustLine(ltx, getPublicKey(), asset);
    REQUIRE(trustLine);
...
TestAccount::getAvailableBalance() const
{
    ...
    return caiz::getAvailableBalance(header, entry);
}
...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto entry = caiz::loadAccount(ltx, publicKey);
        if (entry)
        {
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto destAfter = caiz::loadAccount(ltx, publicKey);
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        REQUIRE(caiz::loadAccount(ltx, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(caiz::loadAccount(ltx, into));
    REQUIRE(!caiz::loadAccount(ltx, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    auto data = caiz::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
...
TestAccount::bumpSequence(SequenceNumber to)
{
    ...
    {
        auto account = caiz::loadAccount(ltx, getPublicKey());
        REQUIRE(account);
                                    ...
                                    xdr::xvector<Claimant, 10> const& claimants)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto entry = caiz::loadClaimableBalance(ltx, returnedBalanceID);
    REQUIRE(entry);
...
TestAccount::claimClaimableBalance(ClaimableBalanceID const& balanceID)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(!caiz::loadClaimableBalance(ltx, balanceID));
}
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountEntry = caiz::loadAccount(ltx, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountAfter = caiz::loadAccount(ltx, destination);
        // check that the target account didn't change
