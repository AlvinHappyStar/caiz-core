...
applyTx(TransactionFramePtr const& tx, Application& app, bool checkSeqNum)
{
    ...
    LedgerTxn ltx(app.getLedgerTxnRoot());
    auto account = caiz::loadAccount(ltx, tx->getSourceID());
...
{
    auto res = caiz::loadAccountWithoutRecord(ltx, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    return (bool)caiz::loadAccountWithoutRecord(ltx, k);
}
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    auto account = caiz::loadAccount(ltx, k);
                  ...
                  uint32_t fee)
{
    ...
    caiz::shuffle(ops.begin(), ops.end(), autocheck::rng());
                       ...
                       Price const& price, int64_t amount, SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerTxn ltx(app.getLedgerTxnRoot());
        auto offer =
            caiz::loadOffer(ltx, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveSellOfferResult.code() == MANAGE_SELL_OFFER_SUCCESS)
    {
        ...
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            REQUIRE(!caiz::loadOffer(ltx, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
