...
cereal_override(cereal::JSONOutputArchive& ar, const xdr::opaque_array<N>& s,
                const char* field)
{
    xdr::archive(ar, caiz::binToHex(caiz::ByteSlice(s.data(), s.size())),
                 field);
}
...
void cereal_override(cereal::JSONOutputArchive& ar, const caiz::PublicKey& s,
                     const char* field);

void cereal_override(cereal::JSONOutputArchive& ar,
                     const caiz::MuxedAccount& muxedAccount,
                     const char* field);

void cerealPoolAsset(cereal::JSONOutputArchive& ar, const caiz::Asset& asset,
                     const char* field);
...
template <typename T>
typename std::enable_if<std::is_same<caiz::Asset, T>::value ||
                        std::is_same<caiz::TrustLineAsset, T>::value ||
                        std::is_same<caiz::ChangeTrustAsset, T>::value>::type
                ...
                const char* field)
{
    ...
    case caiz::ASSET_TYPE_CREDIT_ALPHANUM12:
    {
        ...
        std::string code;
        if (asset.type() == caiz::ASSET_TYPE_CREDIT_ALPHANUM4)
        {
            caiz::assetCodeToStr(asset.alphaNum4().assetCode, code);
        }
        else
        {
            caiz::assetCodeToStr(asset.alphaNum12().assetCode, code);
        }

        xdr::archive(ar, code, "assetCode");
        xdr::archive(ar, caiz::getIssuer(asset), "issuer");
        ar.finishNode();
        break;
    }
    default:
