...
TrustLineWrapper::NonIssuerImpl::addBalance(LedgerTxnHeader const& header,
                                            int64_t delta)
{
    return caiz::addBalance(header, mEntry, delta);
}
...
{
    return caiz::getBuyingLiabilities(header, mEntry);
}
...
TrustLineWrapper::NonIssuerImpl::getSellingLiabilities(
    LedgerTxnHeader const& header)
{
    return caiz::getSellingLiabilities(header, mEntry);
}
...
{
    return caiz::addBuyingLiabilities(header, mEntry, delta);
}
...
TrustLineWrapper::NonIssuerImpl::addSellingLiabilities(
    LedgerTxnHeader const& header, int64_t delta)
{
    return caiz::addSellingLiabilities(header, mEntry, delta);
}

bool
TrustLineWrapper::NonIssuerImpl::isAuthorized() const
{
    return caiz::isAuthorized(mEntry);
}

bool
TrustLineWrapper::NonIssuerImpl::isAuthorizedToMaintainLiabilities() const
{
    return caiz::isAuthorizedToMaintainLiabilities(mEntry);
}

bool
TrustLineWrapper::NonIssuerImpl::isClawbackEnabled() const
{
    return caiz::isClawbackEnabledOnTrustline(mEntry);
}
...
{
    return caiz::getAvailableBalance(header, mEntry);
}
...
TrustLineWrapper::NonIssuerImpl::getMaxAmountReceive(
    LedgerTxnHeader const& header) const
{
    return caiz::getMaxAmountReceive(header, mEntry);
}
