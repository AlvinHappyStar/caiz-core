# What is a ledger?
A ledger represents the state of the Caiz universe at a given point in time.
...
[`src/xdr/Caiz-ledger.x`](../xdr/Caiz-ledger.x)
...
During consensus, nodes work together to decide on the value of CaizValue.
CaizValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'CaizValue' should be able to transition to the same new ledger.
...
[`src/xdr/Caiz-ledger-entries.x`](../xdr/Caiz-ledger-entries.x)

### AccountEntry
This entry represents an account. In Caiz, everything is centered around
...
Offers are a way to automate simple trading inside the Caiz network.
