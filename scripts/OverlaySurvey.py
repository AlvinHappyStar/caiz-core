...
def check_results(data, graph, merged_results):
    if "topology" not in data:
        raise ValueError("caiz-core is missing survey nodes."
...
def get_tier1_stats(augmented_directed_graph):
    '''
    Helper function to help analyze transitive quorum. Must only be called on a graph augmented with CaizBeat info
    '''
...
def augment(args):
    ...
    data = requests.get("https://api.caizbeat.io/v1/nodes").json()
    transitive_quorum = requests.get(
        "https://api.caizbeat.io/v1/").json()["transitiveQuorumSet"]
