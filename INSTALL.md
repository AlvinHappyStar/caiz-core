...
These are instructions for building caiz-core from source.

For a potentially quicker set up, the following projects could be good alternatives:

* caiz-core in a [docker container](https://github.com/caiz/docker-caiz-core)
* caiz-core and [horizon](https://github.com/caiz/go/tree/master/services/horizon) in a [docker container](https://github.com/caiz/docker-caiz-core-horizon)
* pre-compiled [packages](https://github.com/caiz/packages)
...
Best is to use the latest *stable* release that can be downloaded from https://github.com/caiz/caiz-core/releases
...
`caiz-core` does not have many dependencies.
...
#### Ubuntu 20.04
You can install the [test toolchain](#adding-the-test-toolchain) to build and run caiz-core with the latest version of the llvm toolchain.
...
- `git clone https://github.com/caiz/caiz-core.git`
- `cd caiz-core`
- `git submodule init`
...
Configuring with `--enable-tracy` will build and embed the client component of the [Tracy](https://github.com/wolfpld/tracy) high-resolution tracing system in the `caiz-core` binary.

The tracing client will activate automatically when caiz-core is running, and will listen for connections from Tracy servers (a command-line capture utility, or a cross-platform GUI).
